...
import { REGEXP, INVOICE_STATUS } from '../../../utils/constants'

const ALLOWED_STATUS = Object.values(INVOICE_STATUS)
...
class InvoiceValidations {
  ...
  list = (req: Request, res: Response, next: NextFunction) => {
    const { page, limit, status, project_uuid, sales_order_uuid, client_uuid, date_from, date_to } = req.query
    ...
    if (sales_order_uuid && !isUuid(sales_order_uuid)) {
      errors.sales_order_uuid = res.__('VALIDATIONS.invalid', 'sales_order_uuid')
    }
  ...
  create = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!isEmpty(sales_order_uuid) && !isUuid(sales_order_uuid)) {
      errors.sales_order_uuid = res.__('VALIDATIONS.invalid', 'sales_order_uuid')
    }
  ...
  update = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (sales_order_uuid !== undefined && sales_order_uuid !== null && String(sales_order_uuid).trim() !== '' && !isUuid(sales_order_uuid)) {
      errors.sales_order_uuid = res.__('VALIDATIONS.invalid', 'sales_order_uuid')
    }
...
export default new InvoiceValidations()
